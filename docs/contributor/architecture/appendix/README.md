...
1. The Pod runs within a namespace listed in the webhook's default configuration. All Pods in such namespaces are automatically intercepted and modified. This option is primarily used for Kyma-managed namespaces (for example, `kyma-system` and `istio-system`).
...
A dedicated controller loop handles the synchronization of shared resources (for example, pull secret, `ClusterTrustBundle`, or webhook configuration). For more information, see [Runtime Configuration Synchronization Using Controller Loop](./resource-synchronization.md).
