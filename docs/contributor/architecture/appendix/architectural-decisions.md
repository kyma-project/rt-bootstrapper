...
**Components:**
 ...
 3. **RT Bootstrapper:** Modifies Pod manifests and applies landscape-specific adjustments (for example, adding a pull secret or rewriting image-registry hostnames).
...
The Kyma backend ensures that such resources are synchronized from Kyma Control Plane (KCP) to the Kyma runtime `kyma-system` namespace. For more information on this mechanism, see [Runtime Configuration Synchronization Using Controller Loop](./resource-synchronization.md).
