...
| Mount Cluster Trust Bundle Volume | Mount a certificate (stored as `ClusterTrustBundle`) as a projected volume into the container under the path `/etc/ssl/certs` (includes init-containers). Automatically restarts Pods when the CA bundle changes. | Mount a projected `volume` from `ClusterTrustBundle` to each container in the Pod under path `/etc/ssl/certs`. Stamp a CTB hash annotation for change detection. | 1. Add projected volume `rt-bootstrapper-certs` to `.spec.volumes[]`<br/>2. Mount this volume into each container under the mount path `/etc/ssl/certs` by extending the array `.spec.containers[*].volumeMounts`<br/>3. Set `rt-bootstrapper.kyma-project.io/ctb-hash` annotation with the SHA-256 of the current CA bundle | `rt-cfg.kyma-project.io/add-cluster-trust-bundle: "true"` (use `"false"` to explicitly opt out) |
...
3. Mount the `ClusterTrustBundle` as a projected volume and stamp the CTB hash.
...
metadata:
  ...
  annotations:
    ...
    rt-bootstrapper.kyma-project.io/ctb-hash: "a1b2c3..."     # SHA-256 of current CA bundle
    rt-cfg.kyma-project.io/add-cluster-trust-bundle: "true"
    rt-cfg.kyma-project.io/add-img-pull-secret: "true"
    rt-cfg.kyma-project.io/alter-img-registry: "true"
    rt-cfg.kyma-project.io/set-fips-mode: "true"
